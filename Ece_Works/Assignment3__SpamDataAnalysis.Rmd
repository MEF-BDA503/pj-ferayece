...
### 2. Build a Model 
+ Build the model with the training data and show it.
```{r message=FALSE,warning=FALSE}
#build the model.
...
+ To validate the model I use the printcp and plotcp functions. ‘CP’ stands for Complexity Parameter of the tree. The value of cp should be least, so that the cross-validated error rate is minimum. Plotcp() provides a graphical representation to the cross validated error summary. The cp values are plotted against the geometric mean to depict the deviation until the minimum value is reached.
